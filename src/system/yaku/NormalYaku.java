...
public enum NormalYaku implements Yaku {
	...
	PINHU("平和", 1, false, false, false, true) {
		...
		public boolean check(AgariParam agParam, CheckParam param, Field field) {
			...
			for (Mentsu mentu : param.getMentuList()) {
				if (mentu.type() != Mentsu.Type.SYUNTU) {
					return false;
				}
			}
	...
	SANSYOKUDOJUN("三色同順", 2, true, true, false, true) {
		...
		@Override
		public boolean check(AgariParam agParam, CheckParam param, Field field) {
			List<Mentsu> mlist = param.getMentuList();
			ArrayList<Integer> idList = new ArrayList<Integer>();
			for (Mentsu mentu : mlist) {
				if (mentu.type() == Mentsu.Type.SYUNTU) {
					int minId = MyMath.min(mentu.get(0).type().id(), mentu.get(1).type().id(), mentu.get(2).type().id());
					idList.add(minId);
				}
			}
			if (idList.size() < 3)
	...
	IKKI("一気通貫", 2, true, true, false, true) {
		...
		public boolean check(AgariParam agParam, CheckParam param, Field field) {
			...
			if(agParam.isNaki()) {
				...
				for (Mentsu mentu : mlist) {
					if (mentu.type() == Mentsu.Type.SYUNTU) {
						int minId = MyMath.min(mentu.get(0).type().id(), mentu.get(1).type().id(),
								mentu.get(2).type().id());
						idList.add(minId);
					}
				}
	...
	TYANTA("混全帯么九", 2, true, true, false, true) {
		...
		public boolean check(AgariParam agParam, CheckParam param, Field field) {
			...
			for (Mentsu mentu : mlist) {
				if (mentu.type() == Mentsu.Type.SYUNTU) {
					int minNum = MyMath.min(mentu.get(0).number(), mentu.get(1).type().number(), mentu.get(2).type().number());
	...
	JUNTYAN("純全帯么九", 3, true, true, false, true) {
		...
		public boolean check(AgariParam agParam, CheckParam param, Field field) {
			...
			for (Mentsu mentu : mlist) {
				if (mentu.type() == Mentsu.Type.SYUNTU) {
					int minNum = MyMath.min(mentu.get(0).number(), mentu.get(1).number(), mentu.get(2).number());
					if (minNum != 1 && minNum != 7)
						return false;
				} else {
