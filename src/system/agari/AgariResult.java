...
/**
 * ある一人のプレイヤーのあがったときの結果を表すクラス．役セットや点数などを持つ.
...
public class AgariResult {
	...
	public static class Builder {
		 ...
		 */
		private AgariResult checkYakuOnMentsuFetchedEnd(HurohaiList hlist, List<Mentsu> fetchedMentu, CheckParam param, AgariParam agp) {
			// この時点では面子の取り出しは終了している
			Map<Mentsu, CheckParam> mentumap = new HashMap<Mentsu, CheckParam>();

			// あがり牌を含むすべての面子を確認する
			for (Mentsu mentu : fetchedMentu) {
				if (mentu.contains(agp.getAgarihai().type())) {
					List<Mentsu> mentulist = new ArrayList<Mentsu>(fetchedMentu);
					mentulist.addAll(hlist);

					// ロンあがりの場合はその面子を明順子(または明刻子)にする.
					if (!agp.isTsumo()) {
						mentulist.remove(mentu);
						mentulist.add(new Mentsu(mentu, hoju, agp.getAgarihai()));
					}
			...
			Mentsu mentu = getMaxScoreMentu(mentumap);
		...
		private int checkMatiType(CheckParam param, AgariParam agp) {
			...
			if (pinhu) {
				...
				for (Mentsu m : param.getMentuList()) {
					hu += m.calcHu();
				}
		 ...
		 */
		private Mentsu getMaxScoreMentu(Map<Mentsu, CheckParam> mentumap) {
			int max = 0;
			Mentsu ret = null;
			for (Mentsu m : mentumap.keySet()) {
		...
		public void calcHanAndHu(List<HaiType> odora, List<HaiType> udora) {
			...
			// それ以外の場合
				...
				for (Mentsu m : chParam.getMentuList()) {
					this.hu += m.calcHu();
				}
