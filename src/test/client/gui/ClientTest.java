...
import test.server.game.MahjongGame;
...
public class ClientTest {
	public static void main(String[] args) {
		...
		final GameManager gmanager = new GameManager();
		gmanager.setSender(csend1);
		
		final ClickableManager cmanager = new ClickableManager();
		cmanager.setGameManager(gmanager);
		
		final GuiClient gui = new GuiClient();
		gui.setGameManager(gmanager);
		
		ClientListener listener1 = gmanager;
		...
		playerList.add(p3);
		playerList.add(p4);
		final MahjongGame game = new MahjongGame(playerList, rule, sender, receivers, aiMap);
